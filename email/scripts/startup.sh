...
if [ ! -z "$EMAIL_DB_SEED_DATA" ]; then
  ...
  mysql -h $ADMIN_HOST -u $ADMIN_USERNAME -p$ADMIN_PASSWORD -e "use $EMAIL_DB_NAME; GRANT SELECT, INSERT, UPDATE, DELETE ON $EMAIL_DB_NAME.* TO '$DB_USERNAME'@'%' IDENTIFIED BY '$DB_PASSWORD'; FLUSH PRIVILEGES;"
